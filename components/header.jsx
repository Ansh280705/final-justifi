        ...
        <div className={`flex items-center justify-between px-6 py-2 rounded-[28px] border transition-all duration-500 ${scrolled ? 'bg-white/90 border-slate-200/50 shadow-[0_8px_32px_rgba(0,0,0,0.05)] backdrop-blur-xl' : 'bg-transparent border-transparent'}`}>
          ...
          {!isMobile && (
            <nav className="flex items-center gap-10 text-[15px] font-black text-slate-500/80">
              ...
              <Link href="/blog" className="hover:text-navy transition-colors">{t("legalNotification")}</Link>
              <Link href="/about" className="hover:text-navy transition-colors">{t("howItWorks")}</Link>
              {user ? (
                <Link 
                  href={user.role === "LAWYER" ? "/lawyer" : user.role === "ADMIN" ? "/admin" : "/cases"} 
                  className="hover:text-navy transition-colors"
                >
                  {t("myDashboard")}
                </Link>
              ) : (
                <Link href="/cases" className="hover:text-navy transition-colors">{t("myDashboard")}</Link>
              )}
            </nav>
          )}
    ...
    <SheetContent side="right" className="w-full sm:w-[350px] border-l border-slate-100 bg-white/95 backdrop-blur-2xl">
      <div className="pt-12 px-6">
        ...
        <nav className="flex flex-col gap-8">
          ...
          <SheetClose asChild><Link href="/blog" className="text-2xl font-black text-navy border-b border-slate-50 pb-4">Legal Notification</Link></SheetClose>
          ...
          <SheetClose asChild>
            <Link 
              href={user?.role === "LAWYER" ? "/lawyer" : user?.role === "ADMIN" ? "/admin" : "/cases"} 
              className="text-2xl font-black text-navy border-b border-slate-50 pb-4"
            >
              My Dashboard
            </Link>
          </SheetClose>
