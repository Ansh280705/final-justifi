...
export default function VideoCall({ sessionId, token, caseEndTime, caseId, caseStartTime }) {
  ...
  // Timer Effect
  useEffect(() => {
    if (!caseEndTime || !caseStartTime) return;

    const calculateTime = () => {
      const start = new Date(caseStartTime).getTime();
      const end = new Date(caseEndTime).getTime();
      const now = Date.now();
  ...
  }, [caseEndTime, caseStartTime, userRole, router, countdown]);
  ...
  const endCall = () => {
    ...
    if (userRole === "LAWYER") {
      setShowLegalAdvice(true); // Show modal
    } else {
      router.push("/cases"); // Client redirect
    }
  };
  ...
  return (
    <>
      ...
      {showLegalAdvice && (
        <LegalAdviceModal
          legalCase={legalCase}
          lawyer={{ name: userName, id: userRole }}
          onClose={() => setShowLegalAdvice(false)}
        />
      )}
