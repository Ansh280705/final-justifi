...
export default function DisputeDashboardPage() {
  ...
  const handleGenerateMeet = async () => {
    ...
    try {
      ...
      if (data.success) {
        setDispute(prev => ({ ...prev, status: "MEDIATION", meetLink: data.meetLink }));
        alert("Mediation link generated and emailed to both parties!");
      } else {
              ...
              <CardContent className="pt-4 space-y-3">
                ...
                {dispute.status === "NOTICE_SENT" && (
                  <>
                    ...
                    <p className="text-sm text-gray-600 mb-2">Legal notice has been emailed. If you are ready to mediate, generate a video link.</p>
                    <Button 
                ...
                {dispute.meetLink && (
                  <div className="p-3 bg-blue-50 border border-blue-100 rounded-md mb-2">
                    <p className="text-xs font-semibold text-blue-800 mb-1">Mediation Link</p>
                    <a href={dispute.meetLink} target="_blank" className="text-sm text-blue-600 break-all hover:underline flex items-center gap-1">
                      <Video className="h-3 w-3" /> Join Call
                    </a>
                  </div>
                )}

                {(dispute.status === "NOTICE_SENT" || dispute.status === "MEDIATION") && (
                  <div className="pt-4 border-t space-y-2 mt-4">
                    <p className="text-sm text-gray-600 font-semibold">Resolution</p>
                    <Button 
                      className="w-full bg-emerald-600 hover:bg-emerald-700" 
