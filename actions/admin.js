...
export async function getPendingPayouts() {
  ...
  try {
    try {
      ...
      // to a safer query that selects only guaranteed columns from the lawyer.
      if (innerError?.code === "P2022") {
        console.warn(
          "Schema mismatch when fetching pending payouts, falling back to minimal lawyer select",
