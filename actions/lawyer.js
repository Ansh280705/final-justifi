...
export async function getLawyerSpecializedDisputes() {
  const { userId } = await auth();
  if (!userId) throw new Error("Unauthorized");
  ...
  try {
    ...
    if (!lawyer) {
      return { success: true, specializedDisputes: [] };
    }

    const specializedDisputes = await db.dispute.findMany({
      where: {
        seniorLawyerId: lawyer.id,
    ...
    console.error("Failed to fetch specialized disputes:", error);
    return { success: false, error: error.message };
  }
}
