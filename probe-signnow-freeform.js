...
async function run() {
  ...
  if (uploadRes.status === 200 && uploadRes.body.id) {
     ...
     // Skip adding fields. Try freeform invite directly.
     const inviteRes = await makeRequest(`/document/${docId}/invite?free_form=true`, 'POST', {
       to: [{ email: "test@example.com", role_id: "", role: "Signer", order: 1 }],
       from: "admin@justifi.in"
     });
     
     console.log("Freeform Invite:", inviteRes.status, JSON.stringify(inviteRes.body, null, 2));
  }
}
